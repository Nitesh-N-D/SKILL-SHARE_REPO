...
export default function CreateGroupChatPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 sm:p-6 md:p-10">
      ...
      {/* ================= GROUP NAME ================= */}
      <input
        className="w-full border p-3 rounded-lg mb-6 text-sm sm:text-base"
        placeholder="Group Name"
        value={groupName}
        onChange={(e) => setGroupName(e.target.value)}
      />
      ...
      <div className="relative mb-4">
        <Search className="absolute left-3 top-3 text-gray-400" size={18} />
        <input
          className="w-full border pl-10 pr-4 py-2 rounded-lg text-sm sm:text-base"
          placeholder="Search users..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
        />
      ...
      {/* ================= USER LIST ================= */}
      <div className="space-y-2 mb-6 max-h-[300px] sm:max-h-[350px] overflow-y-auto border rounded-lg p-3">
        {filteredUsers.map((u) => (
          <div
            key={u.uid}
            onClick={() => toggleUser(u.uid)}
