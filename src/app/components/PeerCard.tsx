...
export function PeerCard({
  ...
  return (
    ...
    >
      {/* MATCH BADGE */}
      <div className="absolute top-4 right-4 px-3 py-1 rounded-full text-sm font-semibold bg-green-100 text-green-700">
        {peer.matchScore}% Match
      </div>

      {/* AVATAR */}
      <div className="flex justify-center mt-2">
        <div className="rounded-full p-1 bg-gradient-to-br from-blue-500 to-purple-600">
          <Avatar className="size-16 bg-white">
            <AvatarImage src={peer.avatar} />
            <AvatarFallback className="text-lg font-bold text-gray-700">
              {peer.name[0]}
      ...
      <Button
        ...
        onClick={() => onConnect(peer.id)}
        className={`
          w-full mt-6 h-12 text-base font-semibold rounded-xl
          ${
            status === "none"
