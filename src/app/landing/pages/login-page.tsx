...
export function LoginPage({ onBack }: LoginPageProps): JSX.Element {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-pink-50 flex items-center justify-center px-4 sm:px-6 py-8">
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
        ...
        >
          ...
          <h2 className="text-3xl lg:text-4xl mt-6 font-bold text-purple-600">
            Welcome Back!
          </h2>
          ...
          {[ShieldCheck, Users, Sparkles].map((Icon, i) => (
            <div key={i} className="flex items-center gap-3 mt-4 text-sm lg:text-base">
        ...
        {/* RIGHT */}
        <Card className="p-6 sm:p-8 rounded-3xl shadow-2xl bg-white w-full max-w-md mx-auto">
          ...
          <Button
            disabled={loading}
            className="w-full h-12 sm:h-14 bg-gray-900 text-white hover:bg-black text-sm sm:text-base"
            onClick={async () => {
              await signInAnon();
              navigate("/onboarding");
            }}
